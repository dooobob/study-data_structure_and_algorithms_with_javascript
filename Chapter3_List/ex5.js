...
function checkIn(name, movie, movieList, customerList, rentedList) {
    ...
    for (customerList.front(); customerList.currPos() < customerList.length(); customerList.next()) {
        ...
        if (customerList.currPos() === customerList.length() - 1)
            break;
    }
    ...
    if (findCustomer) {
        ...
        console.log(name + " check in " + movie + ".");
    } else {
        console.log(name + " had not rented " + movie + ".");
    }
}
...
console.log("\nCheckIn the Movie\n");
checkIn("Jane Doe", "The Godfather", movieList, customers, rentedMovie);
checkIn("Julia Doe", "The Godfather", movieList, customers, rentedMovie);
console.log("\nCustomer Rentals: \n");
displayList(customers);
console.log("\nRented Movies: \n");
displayList(rentedMovie);